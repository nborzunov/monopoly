...
import { Game, GameFactory } from 'src/libs/game'
...
export class GamesService {
  ...
  async createGame(owner: any, newGame: any, userIsSure: boolean = false) {
  ...
  connectToGame(user: any, gameId: string, userIsSure: boolean = false) {
  ...
  leaveGame(user: any, userIsSure: boolean = false) {
    const userRoom = this.checkUserInRoom(user)

    if (userIsSure && userRoom) {
      userRoom.players = userRoom.players.filter(
