...
export class GamesGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleDisconnect(@ConnectedSocket() socket: Socket) {
    try {
      ...
      console.log(
        'User with email',
        user.email,
        'disconnected from socket "games"'
      )
    } catch (err) {}
  }

  @SubscribeMessage('current_games')
  async currentGames(@MessageBody() data: any) {
  ...
  async createGame(@ConnectedSocket() socket: Socket, @MessageBody() res: any) {
    ...
    try {
      const games = await this.gamesService.createGame(
        user,
        res.data.game,
        res.data.userIsSure
      )
  ...
  async connectToGame(
    ...
    try {
      const games = await this.gamesService.connectToGame(
        user,
        res.data.gameId,
        res.data.userIsSure
      )
